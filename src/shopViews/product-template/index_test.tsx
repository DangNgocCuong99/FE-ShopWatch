...
export const renderProductTest = (dataApi: IProduct[],setListProducts: (value: SetStateAction<IProduct[] | undefined>) => void, column?: number) => {
  ...
  const handleFaverite = async (id: string) => {
    try {
      ...
      setListProducts((product)=> product?.map((val,i)=>{
        if  (val._id === id){
